...
class SmartTradingBot:
    def __init__(self):
        ...
        self.trading_client = TradingClient(
            api_key=self.api_key, secret_key=self.api_secret, paper=True  # Always use paper trading for safety
        )
        ...
        if self.ai.is_configured:
            ...
            logging.info(
                f"🤖 Active AI Provider: {provider_status['current_provider'].upper() if provider_status['current_provider'] else 'None'}"
            )
    ...
    def configure_ai_usage(
        self, ticker_analysis: bool = True, ticker_selection: bool = True, market_summary: bool = True
    ):
        """
...
CREATE POLICY "Allow all operations" ON trades FOR ALL USING (true);"""
    ...
    def get_ai_recommended_tickers(self, portfolio_analysis: Dict) -> List[str]:
        ...
        try:
            ...
            if recently_researched:
                ...
                logging.info(
                    f"🔍 Recently researched tickers to EXCLUDE ({len(recently_researched)}): {', '.join(recently_researched[:20])}"
                )
            else:
            ...
            try:
                ...
                if isinstance(response, dict) and "recommended_tickers" in response:
                    ...
                    print(
                        f"📊 Recommended Tickers ({len(tickers)}): {', '.join(tickers[:10])}{'...' if len(tickers) > 10 else ''}"
                    )
                    ...
                    if filtered_out:
                        ...
                        logging.warning(
                            f"⚠️  AI recommended {len(filtered_out)} tickers in cooldown (ignored instructions): {list(filtered_out)[:10]}"
                        )

                    # If too few tickers after filtering, get fresh ticker list
                    if len(cooldown_filtered_tickers) < 10:
                        logging.warning(
                            f"⚠️  Only {len(cooldown_filtered_tickers)} tickers available after cooldown filter"
                        )
                        logging.info(
                            "🔄 Generating fresh ticker list excluding cooldown, orders, and portfolio positions..."
                        )
                        cooldown_filtered_tickers = self._get_fresh_ticker_list(target_count=30)

                    print(
                        f"📊 AI recommended {len(tickers)} tickers, {len(cooldown_filtered_tickers)} available after filtering"
                    )
                    logging.info(
                        f"📊 AI recommended {len(tickers)} tickers, {len(cooldown_filtered_tickers)} available after filtering"
                    )
                    ...
                    if response is None:
                        logging.warning(
                            "⚠️ AI returned None response (likely API failure or safety filter), using smart fallback"
                        )
                        ...
                        logging.warning(
                            f"⚠️ AI returned dict missing required keys {missing_keys}. Available keys: {available_keys}, using smart fallback"
                        )
                    else:
                        logging.warning(
                            f"⚠️ AI returned unexpected type {response_type}: {str(response)[:100]}, using smart fallback"
                        )
                    return self._get_smart_fallback_tickers(portfolio_analysis)
    ...
    def _get_smart_fallback_tickers(self, portfolio_analysis: Dict) -> List[str]:
        ...
        print(
            f"⏰ Recently researched ({len(recently_researched)}): {', '.join(recently_researched[:5])}{'...' if len(recently_researched) > 5 else ''}"
        )
        ...
        logging.info(
            f"📊 Smart fallback selected {len(selected)} tickers, {len(cooldown_filtered_tickers)} available after filtering"
        )
        return cooldown_filtered_tickers
        ...
        try:
            ...
            for asset in assets:
                try:
                    if (
                        asset.tradable
                        and str(asset.status) == "AssetStatus.ACTIVE"
                        and str(asset.asset_class) == "AssetClass.US_EQUITY"
                    ):
    ...
    def has_pending_orders(self, symbol: str) -> bool:
        ...
        try:
            ...
            pending_orders = [order for order in orders if order.status in open_statuses and order.symbol == symbol]
    ...
    def execute_trade(self, analysis: Dict) -> bool:
        ...
        try:
            ...
            if signal == "SELL":
                try:
                    ...
                    if not position or float(position.qty) <= 0:
                        logging.info(
                            f"🚫 Cannot SELL {symbol}: No position found (qty: {float(position.qty) if position else 0})"
                        )
                        return False
                ...
                if not within_limits:
                    if adjusted_quantity <= 0:
                        logging.info(
                            f"🚫 Skipping {symbol}: Would exceed 10% position limit (currently {position_percentage:.1f}%)"
                        )
                        return False
                    else:
                        logging.info(
                            f"⚠️ Reducing {symbol} quantity from {quantity} to {adjusted_quantity} shares (position limit: {position_percentage:.1f}%)"
                        )
                ...
                if current_position_qty > 0:
                    ...
                    logging.info(
                        f"📊 {symbol} Position: Currently {current_position_qty} shares, adding {quantity} → {new_total_qty} total"
                    )
            ...
            print(f"\n📈 TECHNICAL ANALYSIS:")
            print(
                f"   RSI: {analysis['rsi']:.1f} | SMA Fast: ${analysis['sma_fast']:.2f} | SMA Slow: ${analysis['sma_slow']:.2f}"
            )
            ...
            if signal == "SELL":
                try:
                    ...
                    print(
                        f"📊 POSITION: Had {float(position.qty)} shares, selling {quantity}, keeping {remaining_shares}"
                    )
                except:
                    pass
            ...
            if signal == "BUY":
                ...
                print(
                    f"   • RSI ({analysis['rsi']:.1f}): {'OVERSOLD - Good entry point' if analysis['rsi'] < 30 else 'NEUTRAL - Momentum building' if analysis['rsi'] < 70 else 'STRONG MOMENTUM'}"
                )
                ...
                print(
                    f"   • RSI ({analysis['rsi']:.1f}): {'OVERBOUGHT - Good exit point' if analysis['rsi'] > 70 else 'NEUTRAL - Weakness showing' if analysis['rsi'] > 30 else 'OVERSOLD'}"
                )
            print(f"\n💼 Order ID: {order.id}")
            print("✅" + "=" * 58 + "✅")
            ...
            logging.info(
                f"✅ TRADE: {signal} {quantity} {symbol} @ ${price:.2f} (${price * quantity:,.2f}) - Order: {order.id}"
            )
    ...
    def run_analysis(self, max_symbols: int = 50, max_trades: int = 3, use_ai: bool = False):
        ...
        logging.info(
            f"🔍 Starting analysis (max {max_symbols} symbols, max {max_trades} trades, AI: {ai_enabled}{rate_limit_status})"
        )
        ...
        try:
            ...
            positions_in_cooldown = sum(
                1
                for p in all_positions
                if float(p.qty) > 0
                and float(p.market_value) >= 50
        ...
        for candidate in sell_candidates:
            ...
            logging.info(
                f"   📊 Type: {candidate['sell_type']} ({candidate['sell_qty']}/{candidate['current_qty']} shares)"
            )
        ...
        for i, symbol in enumerate(symbols, 1):
            try:
                ...
                if i % 10 == 0:
                    ...
                    logging.info(
                        f"📊 Progress: {i}/{len(symbols)} symbols ({skipped_total} skipped), {opportunities} opportunities, {trades_executed} trades"
                    )
                ...
                if analysis["signal"]:
                    ...
                    if analysis["signal_strength"] == "WEAK":
                        ...
                        print(
                            f"   ⏭️  {symbol}: ⚠️  {analysis['signal']} signal too weak (RSI: {analysis['rsi']:.1f}, needs < 25 or > 75)"
                        )
                        continue
        ...
        if trades_executed_details:
            ...
            if no_trade_reasons["no_signal"] > 0:
                reasons.append(
                    f"{no_trade_reasons['no_signal']} symbols had no buy/sell signals (RSI not in buy/sell zones)"
                )
        ...
        print(
            f"📈 Symbols Analyzed: {symbols_researched} researched, {symbols_skipped_orders} skipped (pending orders), {symbols_skipped_cooldown} skipped (cooldown)"
        )
        if position_sells_executed > 0:
            print(f"💰 Position Management: {position_sells_executed} position sells executed")
        ...
        try:
            ...
            if total_positions > 0:
                ...
                positions_analyzed = total_positions - positions_in_cooldown
                print(
                    f"📊 Position Analysis: {positions_analyzed} analyzed, {positions_in_cooldown} skipped (sell cooldown)"
                )
        except:
            pass
        ...
        if final_portfolio:
            ...
            logging.info(
                f"💵 Cash: ${final_portfolio['cash_available']:,.2f} ({final_portfolio['cash_percentage']:.1f}%)"
            )
        logging.info("=" * 60)
        ...
        if db_info["tables_exist"]:
            ...
            if sessions:
                ...
                for session in sessions:
                    ...
                    print(
                        f"  Session {session['id']}: {start_time[:19]} | {trades} trades | {symbols} symbols | P&L: ${pnl:.2f}"
                    )
            else:
                print("\n📈 No sessions yet")
    ...
    def analyze_portfolio(self):
        ...
        try:
            ...
            position_weights = [
                (p["symbol"], abs(p["market_value"]) / total_market_value * 100) for p in portfolio_data["positions"]
            ]
            ...
            portfolio_analysis = {
                ...
                "cash_percentage": (
                    (portfolio_data["account"]["cash"] / portfolio_data["account"]["portfolio_value"]) * 100
                    if portfolio_data["account"]["portfolio_value"] > 0
                    else 0
                ),
                "win_rate": win_rate,
            ...
            print(
                f"💵 Cash Available: ${portfolio_analysis['cash_available']:,.2f} ({portfolio_analysis['cash_percentage']:.1f}%)"
            )
            print(f"📈 Unrealized P&L: ${portfolio_analysis['total_unrealized_pnl']:,.2f}")
            print(
                f"🎯 Win Rate: {portfolio_analysis['win_rate']:.1f}% ({portfolio_analysis['winners']} winners, {portfolio_analysis['losers']} losers)"
            )
            ...
            if portfolio_analysis["largest_position"][0] != "N/A":
                logging.info(
                    f"🔝 Largest Position: {portfolio_analysis['largest_position'][0]} ({portfolio_analysis['largest_position'][1]:.1f}%)"
                )
            ...
            if portfolio_analysis["cash_percentage"] > 20:
                logging.info(
                    f"   💰 EXCESS CASH - {portfolio_analysis['cash_percentage']:.1f}% available for deployment"
                )
            ...
            if len(portfolio_analysis.get("high_concentration_positions", [])) > 0:
                logging.info(
                    f"   🎯 {len(portfolio_analysis['high_concentration_positions'])} positions >15% of portfolio"
                )
            logging.info("=" * 60)
        ...
        try:
            ...
            for position in portfolio_analysis["positions"]:
                ...
                if position_weight > 15 and position["unrealized_plpc"] > 50:
                    ...
                    if reduce_qty > 0:
                        logging.info(
                            f"🎯 Reducing {position['symbol']} position by {reduce_qty} shares ({position_weight:.1f}% concentration, {position['unrealized_plpc']:.1f}% gain)"
                        )

                        if self._place_portfolio_order(
                            position["symbol"],
                            reduce_qty,
                            "sell",
                            f"Reduce concentration risk - {position_weight:.1f}% of portfolio",
                        ):
            ...
            for position in portfolio_analysis["positions"]:
                if position["unrealized_plpc"] < -10 and abs(position["market_value"]) > 100:
                    ...
                    logging.info(
                        f"🛑 Setting stop loss for {position['symbol']} at ${stop_price:.2f} ({position['unrealized_plpc']:.1f}% loss)"
                    )
            ...
            if cash_percentage > 20:  # More than 20% cash
                ...
                logging.info(
                    f"💰 High cash position ({cash_percentage:.1f}%) - considering deployment of ${deployable_cash:,.2f}"
                )
                actions_taken.append(
                    f"High cash position identified: ${portfolio_analysis['cash_available']:,.2f} ({cash_percentage:.1f}%)"
                )

            # Action 4: Diversification recommendations
            if portfolio_analysis["concentration_risk"] > 60:
                logging.warning(
                    f"⚠️ High concentration risk: {portfolio_analysis['concentration_risk']:.1f}% in top 5 positions"
                )
            ...
            while True:
                ...
                try:
                    ...
                    for trade in loop_trade_details:
                        ...
                        if trade["ai_insight"]:
                            reasoning += (
                                f", AI: {trade['ai_insight'][:30]}{'...' if len(trade['ai_insight']) > 30 else ''}"
                            )
                    ...
                    print(
                        f"✅ Loop #{loop_count} complete: {loop_trades} trades, {loop_opportunities} opportunities ({loop_duration:.1f}s)"
                    )
    ...
    def _show_performance_summary(
        ...
        start_time: datetime,
        ticker_positions: dict = None,
        ticker_transactions: dict = None,
        final: bool = False,
    ):
        ...
        if ticker_positions and len(ticker_positions) > 0:
            ...
            sorted_tickers = sorted(ticker_positions.items(), key=lambda x: abs(x[1]["total_invested"]), reverse=True)
            ...
            for symbol, pos in sorted_tickers:
                ...
                loops_span = (
                    f"Loop {pos['first_loop']}-{pos['last_loop']}"
                    if pos["first_loop"] != pos["last_loop"]
                    else f"Loop {pos['first_loop']}"
                )
                ...
                if ticker_transactions and symbol in ticker_transactions:
                    ...
                    for trade in recent_trades:
                        ...
                        print(
                            f"      {action_emoji} Loop {trade['loop']:2d}: {trade['action']} {trade['quantity']:2.0f} @ ${trade['price']:.2f} (${trade['value']:,.2f})"
                        )
        ...
        if not final and len(performance_data) >= 5:
            ...
            for loop_data in performance_data[-5:]:
                ...
                print(
                    f"   Loop {loop_data['loop_number']:3d} ({timestamp}): {loop_data['trades']} trades, {loop_data['opportunities']} opportunities"
                )
...
def main():
    ...
    try:
        ...
        parser.add_argument(
            "--continuous", "-c", action="store_true", help="Run in continuous mode (default: single session)"
        )
        parser.add_argument(
            "--delay", "-d", type=int, default=300, help="Seconds between loops in continuous mode (default: 300)"
        )
        parser.add_argument(
            "--max-symbols", type=int, default=30, help="Maximum symbols to analyze per loop (default: 30)"
        )
        parser.add_argument("--max-trades", type=int, default=2, help="Maximum trades to execute per loop (default: 2)")
        parser.add_argument("--summary-interval", type=int, default=50, help="Show summary every N loops (default: 50)")

        # AI configuration flags
        parser.add_argument(
            "--no-ai-ticker-analysis",
            action="store_true",
            help="Disable AI analysis for individual tickers (use only technical indicators)",
        )
        parser.add_argument(
            "--no-ai-ticker-selection",
            action="store_true",
            help="Disable AI-based ticker selection (use standard ticker list)",
        )
        parser.add_argument("--no-ai-market-summary", action="store_true", help="Disable AI market sentiment summaries")
        parser.add_argument(
            "--no-ai", action="store_true", help="Disable ALL AI features (pure technical analysis mode)"
        )
