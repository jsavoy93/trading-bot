...
(function() {
  ...
  const renderChatHistory = (history) => {
    ...
    const html = messages.map((message, index) => {
      ...
      const truncatedBadge = isTruncated
        ? '<div class="chat-truncated" role="status" aria-label="Response truncated by dashboard safety bound">Response truncated</div>'
        : '';
  ...
  const sendChatMessage = async (message) => {
    ...
    try {
      ...
      // Accepted-with-runId semantics: the Gateway has queued the run and
      // assigned a run_id; the manager continues asynchronously. The
      // compose controls MUST return to idle immediately and chat.history
      ...
      // accepted submit immediately; the next chat.history poll will
      // reconcile to the authoritative sessionInfo state and will clear
      // any stale FAILED indicator (see setAgentStatus below).
